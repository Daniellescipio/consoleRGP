...
const selectWeapon = (weapon, player, setPlayer, setPlayerActivity, typingFunction)=>{
    if(player.inTheShop.shopping || player.inTheShop.making ){
        if(player.inTheShop.shopping){
            ...
            let anyOthers = []
            let enough = player.inventory.pieces.filter(piece=>{
                for(let i =0; i <weapon.pieces.length;i++){
                    if(weapon.pieces[i].id===piece.id){
                        return piece
                    }else{
                        anyOthers.push(piece)
                    }
                }
            })
            if(enough.length === weapon.pieces.length){
                setPlayer(prev=>({...prev, inTheShop:{shopping:false, making:false},inventory:{...prev.inventory, pieces:anyOthers, weapons:[...prev.inventory.weapons, weapon]}}))
        ...
        if(player.inventory.weapons.findIndex(wpn=>wpn.name === weapon.name)>=0){
            ...
            const newMonsterLife = monster.life - playerDamage < 0 ? 0 : monster.life - playerDamage
                if(newPlayerLife <= 0 ){
                    ...
                    typingFunction( "text",`You're on your last leg, but mama didn't raise a quitter. You ${battleObject.heroAttack}, ${monster.name}. The ${monster.name} ${battleObject.monsterDodge}. Then, ${monster.name} ${monster.finishingMove} It's game over. ${monster.winPhrase} they sneer standing over you.`, true)
    ...
    if (playerActivity === "" || playerActivity === "dragon"){
        if(playerActivity !== "dragon"){
            ...
            if(player.hasKey){
                ...
                setPlayerActivity("encounter")
                setPlayer(prevPlayer=>({...prevPlayer, fighting:fireBreathingDragon}))
                typingFunction ("text",`You creep up to the cave where the dragon is hiding the jey. Your strongest weapon is${player.weapons[player.weapons.length-1]} and as you hear the Dragon bellow you begin to wonder if it's enough... \n Are you sure you wan't to do this?`, true)
            }
        }else{
